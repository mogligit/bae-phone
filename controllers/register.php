<?php
    ...
    // Set empty form variables for validation mapping
    ...
    if(isset($_POST["submit"])) {
        ...
        if(!empty($firstname) && !empty($lastname) && !empty($email) && !empty($mobilenumber) && !empty($password)){
            ...
            if($rowCount > 0) {
                ...
                if(!preg_match("/^[0-9]{11}+$/", $_mobile_number)) {
                    $_mobileErr = '<div class="alert alert-danger">
                            Only 11-digit mobile numbers allowed.
                        </div>';
                }
                ...
                // Store the data in database, if all the preg_match condition met
                if((preg_match("/^[a-zA-Z ]*$/", $_first_name)) && (preg_match("/^[a-zA-Z ]*$/", $_last_name)) &&
                 (filter_var($_email, FILTER_VALIDATE_EMAIL)) && (preg_match("/^[0-9]{11}+$/", $_mobile_number)) &&
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    if($sqlQuery) {
                        $msg = 'Click on the activation link to verify your email with BAE Phone services LTD. <br><br>
                          <a href="http://localhost:80/ssd-cwk1-bae/user_verificaiton.php?token='.$token.'"> Click here to verify email</a>
                        ';

                        // Create the Transport
                        $transport = (new Swift_SmtpTransport('smtp.gmail.com', 465, 'ssl'))
                        ->setUsername('ssdcoursework1@gmail.com')
                        ->setPassword('ssdcoursework123');
